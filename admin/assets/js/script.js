...
// =======================================================
// Panel ID Search — only active on panel.php
// =======================================================
(function () {
  // Check karo ki current page panel.php hai ya nahi
  var isPanelPage = window.location.pathname.toLowerCase().indexOf('panel.php') !== -1;
  if (!isPanelPage) return; // Doosre pages pe kuch nahi karna
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Placeholder change karo panel page pe
    searchInput.placeholder = 'Search Panel ID...';

    // Input event - 300ms debounce ke baad search karo
    searchInput.addEventListener('input', function () {
      clearTimeout(searchDebounce);
      var query = this.value.trim();
      if (query === '') {
        restoreTableOrder(); // Search clear hone pe table restore karo
        return;
      }
      searchDebounce = setTimeout(function () {
        highlightPanelRow(query);
      }, 300);
    });

    // Enter key press pe turant search
  ...
  /**
   * Matching Panel ID row ko table ke TOP pe le jaata hai aur highlight karta hai
   */
  function highlightPanelRow(query) {
    ...
    // Pehle sab rows show karo aur highlighting hata do
    rows.forEach(function (row) {
      row.classList.remove('panel-search-highlight');
      row.style.display = '';
    });

    // Panel ID column index (0=Sr No, 1=Panel ID)
    var PANEL_ID_COL = 1;
    ...
    rows.forEach(function (row) {
      ...
      if (cells.length > PANEL_ID_COL) {
        var cellText = cells[PANEL_ID_COL].textContent.trim();
    ...
    if (matchedRows.length === 0) {
      // Koi match nahi - "no result" row dikhao
      var noResultRow = document.createElement('tr');
      noResultRow.id = 'panel-no-result-row';
      noResultRow.innerHTML = "<td colspan='5' class='text-center text-danger py-3'><i class='ph ph-magnifying-glass me-1'></i> No panel found for \"" + query + "\"</td>";
      tbody.insertBefore(noResultRow, tbody.firstChild);
      return;
    }

    // Purana "no result" row hata do agar tha
    var oldNoResult = document.getElementById('panel-no-result-row');
    if (oldNoResult) oldNoResult.remove();

    // Matched rows ko top pe le jao aur highlight karo
    ...
    var tableContainer = tbody.closest('.table-responsive') || tbody.closest('.card');
    if (tableContainer) {
  ...
  /**
   * Search clear hone pe original order restore karo (fetchData re-call)
   */
  function restoreTableOrder() {
    // panel.php ka fetchData function call karo agar available ho
